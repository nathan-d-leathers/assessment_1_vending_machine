...
def optimal_change(item_cost, amount_paid):
    ...
    price = item_cost
    payment = amount_paid
    optimal_statement = "The optimal change for an item that costs $" + \
        str(f"{price:.2f} ") + "with an amount paid of $" + \
        str(f"{payment:.2f} ") + "is "
    a_tracker = 0

    for (a, b, c) in itertools.zip_longest(num_of_bills_and_coins, total_change, verbage, fillvalue=""):
        if (a_tracker == len(num_of_bills_and_coins) - 1) and (len(num_of_bills_and_coins) != 1):
            answer.append("and " + a)
        else:
            answer.append(a)
            a_tracker += 1
